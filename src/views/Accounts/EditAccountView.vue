<template>
  <v-container class="new-account mt-5 px-10">
    <PageHeader header-text="Edit Account" class="mb-7">
      <template #subheader>
    ...
    <div class="d-flex align-center mb-7">
      <div v-if="!account_fields.logoFile && !account_fields.logoUrl" class="upload-background d-flex align-center justify-center" @click="$refs.logo_container.click()">
      ...
      <div v-else class="upload-background d-flex align-center justify-center">
        <img v-show="account_fields.logoFile" ref="logo_preview" style="max-width: 92px; max-height: 92px;">
        <img v-show="account_fields.logoUrl" :src="account_fields.logoUrl" alt="account logo" style="max-width: 92px; max-height: 92px;">
        <v-icon
    ...
    <validation-observer v-slot="{ invalid }">
      <v-row class="py-0">
        ...
        <v-col cols="12" sm="4" class="py-0">
          <CustomDatePicker
            :date.sync="account_fields.subscriptionStartDate"
      ...
      <v-row class="mt-10">
        <v-col cols="12" sm="6">
          ...
          <div class="ml-4">
            <v-checkbox
              v-for="(item, index) in tactic_lists_with_values"
              :key="index"
              v-model="item.value"
            >
    ...
    <v-dialog v-model="dialog" scrollable max-width="560px">
      <AddOrEditCustomFieldDialog
        ...
        @dismiss="dialog = false; clear_active_custom_field()"
...
export default {
  ...
  data() {
    return {
      ...
      pulse_EDS_account_ID: "8743b52063cd84097a65d1633f5c74f5",
      pulse_database_server: "db3.pulse.webair.com",
      active_custom_field: {},
      dialog: false,
      custom_fields: [],
      accountStatuses: [
  ...
  async created() {
    ...
    // Need to get tactics before setting current account because the tactics need to be available
    // in order to set the values of tactic_lists_with_values correctly in setCurrentAccount
  ...
  methods: {
    ...
    setCurrentAccount(account) {
      ...
      this.custom_fields = JSON.parse(JSON.stringify(account.customFieldDefinition.contact));
      this.custom_fields = this.custom_fields.map(field => ({...field, id: uuidv4()}))
      this.tactic_lists_with_values = this.tactic_lists.map((tactic) => ({
        ...tactic,
        value: account.allowedTactics.includes(tactic.id),
      }));
    },
    removeLogo() {
      this.account_fields.logoFile = null;
      this.account_fields.logoUrl = null;
    },
    ...
    async get_tactics() {
      try {
        ...
        console.error("Load tactics failed", e);
      }
    },
    async save_account() {
      this.account_fields.customFieldDefinition.contact = this.custom_fields
      this.account_fields.allowedTactics = this.tactic_lists_with_values
          .filter(item => item.value)
          .map(item => item.id);

      const data = {
        ...this.account_fields,
        subscriptionTerm: parseInt(this.account_fields.subscriptionTerm),
      };

      await this.$rest.account.put_resource(this.account_fields.id, data)
    ...
    async save_custom_field(field) {
      for (let i = 0; i < this.custom_fields.length; i++) {
        if (field.id === this.custom_fields[i].id) {
          this.$set(this.custom_fields, i, field);
          return;
        }
      }
