    ...
    <v-simple-table>
      ...
      <tbody class="search-table__content">
        <tr v-for="(item, index) in records" :key="index">
            ...
            <div>
              <v-menu :close-on-content-click="false" open-on-hover>
                ...
                <item-details-popup
                  :id="item.contact.id"
                  :key="item.contact.id"
                  :contact="{...item.contact, displayContactEmail: item.contactEmail}"
          ...
          <td>{{ item.contactEmail.emailAddress }}</td>
          <td>{{ item.actionCodeName }}</td>
          <td
            class="search-table__creative"
            @click="goToCampaignEditPage(item)"
          >
            {{ item.campaign.name }}
          </td>
          <td
            class="search-table__creative"
            @click="goToEmailCreativeEditPage(item)"
          >
            {{ item.creativeName }}
          </td>
          <td>
...
export default {
  ...
  methods: {
    ...
    goToEmailCreativeEditPage(record) {
      this.$router.push({
    ...
    async goToCampaignEditPage(record) {
      if (!record.campaign) {
        return;
      }

      try {
        const { data } = await this.$rest.campaign.get_resource(record.campaign.id);
        ...
        this.$store.commit('snackbar/showMessage', {
          content: "Error while trying to redirect you to the Campaign Page, please try again",
          color: "error",
        });
      }
    },
