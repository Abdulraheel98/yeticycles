    ...
    <v-simple-table>
      ...
      <tbody class="search-table__content">
        <tr v-for="(item, index) in records" :key="index">
          ...
          <td>
            {{ item.contactLocation.city }}, {{ item.contactLocation.state }} {{ item.contactLocation.postalCode }}
          </td>
          <td>{{ item.actionCodeName }}</td>
          <td
            class="search-table__creative"
            @click="goToSearchDirectMail(item.creativeName)"
          >
            {{ item.creativeName }}
          </td>
          <td>
...
export default {
  ...
  methods: {
    ...
    goToSearchDirectMail(directMailCreativeName) {
      this.$router.push({
        name: "DirectMail",
        query: {
          keyword: directMailCreativeName,
        },
      });
    }
  },
